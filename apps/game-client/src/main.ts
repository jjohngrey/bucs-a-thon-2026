...
function renderMuteButton(): string {
  ...
  return `
    <div class="top-left-controls">
      <a
        href="https://github.com/jjohngrey/bucs-fighter"
        class="github-btn"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Open BUCS Fighter GitHub repository"
        title="GitHub"
      >
        <svg class="github-btn-icon" viewBox="0 0 16 16" aria-hidden="true">
          <path
            fill="currentColor"
      ...
      <button type="button" class="mute-btn" data-action="toggle-mute" aria-label="${label}" title="${label}">
        <img src="${src}" alt="" class="mute-btn-icon" />
      </button>
    </div>
  `;
}
  ...
  return `
    ...
    <main class="shell lobby-screen">
      <section class="card lobby-card">
        ...
        <section class="lobby-controls-note" aria-label="Control hints">
          <p class="lobby-controls-note-title">Controls</p>
          <p class="lobby-controls-note-text">
            <span class="lobby-controls-key">WASD</span> for movement,
            <span class="lobby-controls-key">J</span> for punch,
            <span class="lobby-controls-key">K</span> for kick, and hold
            <span class="lobby-controls-key">L</span> for Smash attack.
          </p>
        </section>
