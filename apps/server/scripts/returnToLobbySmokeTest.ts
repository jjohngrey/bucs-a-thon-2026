  ...
  try {
    ...
    try {
      ...
      await Promise.all([
        ...
        waitForSnapshotCount(guest, 1),
      ]);

      const hostPlayerId = host.session?.playerId;
      assert.ok(hostPlayerId);

      hostSocket.emit(CLIENT_EVENTS.MATCH_END, {
        roomCode: createdLobby.lobby.roomCode,
        winnerPlayerId: hostPlayerId,
        eliminatedPlayerIds: guest.session?.playerId ? [guest.session.playerId] : [],
      });
      ...
      hostSocket.emit(CLIENT_EVENTS.LOBBY_RETURN, {
        roomCode: createdLobby.lobby.roomCode,
      });

      const [hostReturnedLobby, guestReturnedLobby] = await Promise.all([
        waitForResetLobby(host),
        waitForResetLobby(guest),
      ]);

      assert.equal(hostReturnedLobby.lobby.roomCode, createdLobby.lobby.roomCode);
      assert.equal(guestReturnedLobby.lobby.roomCode, createdLobby.lobby.roomCode);
      assert.equal(hostReturnedLobby.lobby.phase, "waiting");
      assert.equal(guestReturnedLobby.lobby.phase, "waiting");
      assert.ok(hostReturnedLobby.lobby.players.every((player) => player.isReady === false));
      assert.ok(guestReturnedLobby.lobby.players.every((player) => player.isReady === false));
      assert.ok(hostReturnedLobby.lobby.players.every((player) => player.selectedCharacterId === null));
      assert.equal(hostReturnedLobby.lobby.selectedStageId, null);

      console.log("Return-to-lobby smoke test passed.");
...
async function waitForResetLobby(state: TestClientState) {
  return waitForLobby(
    state,
    (payload) =>
      payload.lobby.phase === "waiting" &&
      payload.lobby.selectedStageId === null &&
      payload.lobby.players.every(
        (player) => player.isReady === false && player.selectedCharacterId === null,
      ),
  );
}
